    ...
    else if (m_param->bEnableTemporalSubLayers > 2)
    {
        ...
        // Fill the complete mini GOP when temporal sub layers are enabled
        {
            ...
            if (m_param->bBPyramid && !brefs)
            {
                ...
                {
                    for (int j = 1; j < x265_gop_ra[m_gopId][0].poc_offset; j++)
                    {
                        if (x265_gop_ra[m_gopId][j].isNonBase < 2)
                        {
...
bool Lookahead::scenecut(Lowres **frames, int p0, int p1, bool bRealScenecut, int numFrames)
{
    ...
    if (bRealScenecut && (m_param->bframes || m_param->scenecutThreshold) )
    {
        int origmaxp1 = p0 + 1;
...
{
    /* Only do analysis during a normal scenecut check. */
    if (m_param->bframes || m_param->scenecutThreshold)
    {
        int origmaxp1 = p0 + 1;
        /* Look ahead to avoid coding short flashes as scenecuts. */
        origmaxp1 += m_param->bframes;
        ...
        if (m_param->bBPyramid && next_p - cur_p > 2)
        {
            ...
            else
            {
                if (m_param->bFrameAdaptive == X265_B_ADAPT_AUTO)
                {
                    ...
                    for (int middle = loc; middle < next_p; middle++)
                    {
                        int64_t sub_cost = cost;

                        sub_cost += estGroup.singleCost(cur_p, next_p, middle);

                        if (middle > loc)
                            for (int next_b = loc; next_b < middle && sub_cost < threshold; next_b++)
                                sub_cost += estGroup.singleCost(cur_p, middle, next_b);

                        if (middle < next_p - 1)
                            for (int next_b = middle + 1; next_b < next_p && sub_cost < threshold; next_b++)
                                sub_cost += estGroup.singleCost(middle, next_p, next_b);
                    ...
                    for (int next_b = loc; next_b < next_p && sub_cost < threshold; next_b++)
                        sub_cost += estGroup.singleCost(cur_p, next_p, next_b);
